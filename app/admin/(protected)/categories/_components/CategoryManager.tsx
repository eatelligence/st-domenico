    ...
    <div className="flex flex-col lg:flex-row gap-6">
      <div className="flex-1">
        <div className="bg-white border border-gold/20">
              ...
              {cat.isActive && (
                confirmDelete === cat.id ? (
                  <span className="flex items-center gap-1 text-xs font-inter">
                    <button onClick={() => handleDelete(cat.id)} disabled={isPending} className="text-terracotta hover:underline">Yes</button>
                  ...
                  <button onClick={() => { setConfirmDelete(cat.id); setDeleteError('') }}
                    className="p-2 text-charcoal/20 hover:text-terracotta transition-colors" title="Delete category">
                    <Trash2 size={14} />
                  </button>
                )
              )}
          ...
          {categories.length === 0 && (
            <div className="px-6 py-10 text-center font-inter text-sm text-charcoal/40">No categories yet.</div>
          )}
        </div>
      ...
      <div className="lg:w-72 shrink-0">
        <div className="bg-white border border-gold/20 p-6">
          <h2 className="font-playfair text-lg text-charcoal mb-5">Add category</h2>
